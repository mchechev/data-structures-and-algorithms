...
void solve()
{
    ...
    while (!q.empty())
    {
        ...
        if (current.dist == Distance && used[current.x][current.y] == false)
        { // if it is at the right distance add it to the sum and is not used
            sum += arr[current.x][current.y];
        }
        else
        {
            // push the neighboring vertices into the queue if they are valid destinations
